...
- PDF **全文翻译**（工具栏 Languages，在视觉批注旁）：
  ...
  - **公式与图像保护**：版面模型标出的 `formula` / `formula_number` 框及其相交的 OCR 文本框不送入翻译，保留原 PDF 公式；`image` / `chart` / `table` 框内的 OCR 文本同样跳过，避免译文覆盖层遮住原图。图题 `figure_title` 保持可翻译。当前解析提供区域而非逐字符公式组件，因此混有公式的正文 OCR 框整体跳过，优先保证公式完整。
